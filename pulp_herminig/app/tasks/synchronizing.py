...
from pulp_herminig.app.models import HerminigContent, HerminigRemote
...
def synchronize(remote_pk, repository_pk, mirror):
    ...
    """
    remote = HerminigRemote.objects.get(pk=remote_pk)
    ...
    first_stage = HerminigFirstStage(remote, deferred_download)
    DeclarativeVersion(first_stage, repository, mirror=mirror).create()


class HerminigFirstStage(Stage):
    """
    The first stage of a pulp_herminig sync pipeline.
    """
    ...
    async def run(self):
        ...
        for entry in self.read_my_metadata_file_somehow(result.path):
            unit = HerminigContent(entry)  # make the content unit in memory-only
    ...
    def read_my_metadata_file_somehow(self, path):
        """
        Parse the metadata for herminig Content type.
