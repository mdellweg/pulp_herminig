...
from pulp_herminig.app.serializers import HerminigContentSerializer
from pulp_herminig.app.models import HerminigContent

from pulpcore.plugin.models import Artifact


# Fill data with sufficient information to create HerminigContent
# Provide sufficient parameters to create the HerminigContent object
# Depending on the base class of the serializer, provide either "_artifact" or "_artifacts"
@unittest.skip("FIXME: plugin writer action required")
class TestHerminigContentSerializer(TestCase):
    """Test HerminigContentSerializer."""

    def setUp(self):
        """Set up the HerminigContentSerializer tests."""
    ...
    def test_valid_data(self):
        """Test that the HerminigContentSerializer accepts valid data."""
        data = {"_artifact": "/pulp/api/v3/artifacts/{}/".format(self.artifact.pk)}
        serializer = HerminigContentSerializer(data=data)
        self.assertTrue(serializer.is_valid())

    def test_duplicate_data(self):
        """Test that the HerminigContentSerializer does not accept data."""
        HerminigContent.objects.create(artifact=self.artifact)
        data = {"_artifact": "/pulp/api/v3/artifacts/{}/".format(self.artifact.pk)}
        serializer = HerminigContentSerializer(data=data)
