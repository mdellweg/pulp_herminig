"""Tests that sync herminig plugin repositories."""
import unittest
...
from pulp_herminig.tests.functional.constants import (
    HERMINIG_FIXTURE_SUMMARY,
    HERMINIG_INVALID_FIXTURE_URL,
)
...
class BasicSyncTestCase(unittest.TestCase):
    """Sync a repository with the herminig plugin."""

    @classmethod
    def setUpClass(cls):
        """Create class-wide variables."""
        cls.cfg = config.get_config()
        cls.client = gen_herminig_client()

    def test_sync(self):
        """Sync repositories with the herminig plugin.
        ...
        8. Assert that the same number of are present and that no units were
           added.
        """
        repo_api = RepositoriesHerminigApi(self.client)
        remote_api = RemotesHerminigApi(self.client)
        ...
        self.assertDictEqual(get_content_summary(repo.to_dict()), HERMINIG_FIXTURE_SUMMARY)
        self.assertDictEqual(get_added_content_summary(repo.to_dict()), HERMINIG_FIXTURE_SUMMARY)
...
class SyncInvalidTestCase(unittest.TestCase):
    ...
    @unittest.skip("FIXME: Plugin writer action required.")
    def test_invalid_herminig_content(self):
        ...
        """
        with self.assertRaises(PulpTaskError) as cm:
            task = self.do_test(HERMINIG_INVALID_FIXTURE_URL)
        task = cm.exception.task.to_dict()
    ...
    def do_test(self, url):
        ...
        body = gen_herminig_remote(url=url)
        remote = remote_api.create(body)
        self.addCleanup(remote_api.delete, remote.pulp_href)
